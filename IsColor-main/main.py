...
def generate_inpaint_mask(bw_img):
    """
    仿照generate_inpaint_mask的输入输出封装连通域分割算法
    输入：bw_img - PIL Image对象（RGB格式的黑白图）
    输出：mask - PIL Image对象（3通道RGB格式，分割后的目标掩码：目标=白，背景=黑）
    """
    # 1. 转换为灰度图（对齐原函数的预处理逻辑）
    gray = cv2.cvtColor(np.array(bw_img), cv2.COLOR_RGB2GRAY)

    # 2. 核心分割逻辑（带小域过滤的所有连通域拼接）
    # 中值滤波去噪
    ...
    # 3. 格式转换（对齐原函数输出：3通道RGB的PIL Image）
...
def get_canny_map(input_image_cv2):
    ...
    input_image_cv2 = cv2.Canny(input_image_cv2, 20, 80)
...
if __name__ == "__main__":
    ...
    config = RunConfig(
        ...
        guidance_scale=5.0,        # SD15着色推荐引导强度
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # 8. 生成图像描述
        bw_image_prompt = (
            "microscopic image of Aphanizomenon flos-aquae under bright-field illumination, "
            "filamentous cyanobacterium — long chain of cylindrical cells arranged end-to-end, NOT a complete organism, likely single filament, "
            "visible cell boundaries — faint septa or cross-walls dividing cells, "
            "internal structure — visible granular inclusions or pigment bodies within cells, "
        ...
        ref_image_prompt = (
             "Aphanizomenon flos-aquae under phase-contrast microscopy, "
            "filamentous cyanobacterium — semi-transparent blue-green dominant color, "
            "with occasional yellow-green or orange-yellow pigment patches — possibly phycobiliproteins or carotenoids, "
            ...
            "internal structure — visible pigment granules or empty spaces, NOT uniform, NOT opaque, "
        ...
        # 12. 频率域操作
        latent_h, latent_l, latent_sum = freq_exp(inv_latent, d_s=0.1, d_t=0.5, alpha=0.2, filter_type="gaussian_b")
        ...
        prompt = (
            "high-resolution microscopic image of Aphanizomenon flos-aquae, "
            "FILAMENTOUS CYANOBACTERIUM — LONG CHAIN OF CYLINDRICAL CELLS ARRANGED END-TO-END, MUST be non-spherical, non-circular, NOT polygonal, "
            "CELL BOUNDARIES — VISIBLE FAINT SEPTA OR CROSS-WALLS DIVIDING CELLS — NOT BOLD, NOT SOLID, "
            "INTERNAL STRUCTURE — VISIBLE GRANULAR INCLUSIONS OR PIGMENT BODIES — NOT UNIFORM, NOT GLOWING, "
            ...
            "COLOR — BLUE-GREEN DOMINANT, WITH OCCASIONAL YELLOW-GREEN OR ORANGE-YELLOW PIGMENT PATCHES AT CELL INTERIORS OR WALLS, "
            ...
            "SURFACE — MAY SHOW MINOR TEXTURAL VARIATION (e.g., micro-folds or fine granules), NOT FLAT, NOT POLISHED, "
            
            "HIGH CONTRAST BETWEEN STRUCTURE AND BACKGROUND, "
            "MUST STRICTLY FOLLOW INPUT GRAYSCALE STRUCTURE — DO NOT ADD/REMOVE CELL BOUNDARIES, PIGMENT BODIES, OR BRIGHTNESS GRADIENT, "
        ...
        negative_prompt = (
            ...
            "biological features not matching cyanobacteria — e.g., no cell membrane, no nucleus, no flagella, "
        ...
        # 15. 保存结果
        output_path = os.path.join(config.result_path, f"{save_name}.png")
