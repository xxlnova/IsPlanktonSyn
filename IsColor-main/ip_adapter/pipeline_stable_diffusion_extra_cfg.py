...
class StableDiffusionPipelineCFG(
    DiffusionPipeline,
    ...
    def __call__(
       ...
       #     callback_on_step_end_tensor_inputs,
        ...
        if simple_cfg_mode ==False:
            prompt_embeds, negative_prompt_embeds,img_only_prompt_embeds,img_prompt_everything_cond = self.encode_prompt(
                prompt,
                device,
                num_images_per_prompt,
        ...
        if self.do_classifier_free_guidance:
            ...
            if simple_cfg_mode == True:
                ...
                prompt_embeds = torch.cat([prompt_embeds, img_only_prompt_embeds,img_prompt_everything_cond, negative_prompt_embeds])
                #prompt_embeds = torch.cat([prompt_embeds, img_only_prompt_embeds, negative_prompt_embeds])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                    ...
                    #CHECK THIS LATER
                    noise_pred = rescale_noise_cfg(noise_pred, noise_pred_full, guidance_rescale=self.guidance_rescale)
