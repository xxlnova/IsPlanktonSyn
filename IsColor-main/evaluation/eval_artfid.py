...
'''This code come from StyleID => https://github.com/jiwoogit/StyleID '''
...
# ===================== 路径基础配置（固定前缀，无需修改）=====================
STYLE_BASE_PATH = "/root/autodl-tmp/StyleSSP-main/dataset/DYB-PlanktonNet"
CONTENT_BASE_PATH = "/root/autodl-tmp/StyleSSP-main/dataset/DYB-PlanktonNet-black"
TARGET_BASE_PATH = "/root/autodl-tmp/StyleSSP-main/results"
...
def compute_fid(path_to_stylized, path_to_style, batch_size, device, num_workers=1):
    ...
    ckpt_file = '/root/autodl-tmp/StyleSSP-main/checkpoint/art_inception.pth'
...
def compute_fid_infinity(path_to_stylized, path_to_style, batch_size, device, num_points=15, num_workers=1):
    ...
    # ckpt_file = utils.download(CKPT_URL)
    ckpt_file = '/root/autodl-tmp/StyleSSP-main/checkpoint/art_inception.pth'  # 本地权重路径
    ckpt = torch.load(ckpt_file, map_location=device)
...
def compute_content_distance(path_to_stylized, path_to_content, batch_size, content_metric='lpips', device='cuda',
                             num_workers=1, gray=False):
    ...
    for batch_stylized, batch_content in zip(dataloader_stylized, dataloader_content):
        with torch.no_grad():
            if batch_stylized.shape != batch_content.shape:
...
def compute_art_fid(path_to_stylized, path_to_style, path_to_content, batch_size, device, mode='art_fid_inf',
    ...
    cnt_value = compute_content_distance(path_to_stylized, path_to_content, batch_size, content_metric, device,
                                         num_workers)
    gray_cnt_value = compute_content_distance(path_to_stylized, path_to_content, batch_size, content_metric, device,
                                              num_workers, gray=True)
...
def expand_gray_to_rgb(img):
    if img.mode == 'L':  # 单通道灰度图
        return Image.merge('RGB', (img, img, img))
    return img


def main():
    parser = argparse.ArgumentParser(description='Calculate ArtFID with simplified path input')
    ...
    parser.add_argument('--content_metric', type=str, default='lpips',
                        choices=['lpips', 'vgg', 'alexnet', 'ssim', 'ms-ssim'], help='Content distance.')
    parser.add_argument('--mode', type=str, default='art_fid_inf', choices=['art_fid', 'art_fid_inf'],
                        help='Evaluate ArtFID or ArtFID_infinity.')
    parser.add_argument('--device', type=str, default='cuda', choices=['cuda', 'cpu'], help='Device to use.')
    # 🔥 核心修改：仅需输入一次子文件夹名称
    parser.add_argument('--subdir', type=str, default='001_Polychaeta_most with eggs',
                        help='Subfolder name (only need to input ONCE!)')

    args = parser.parse_args()

    # 自动拼接完整路径（无需手动输入三次！）
    style_path = os.path.join(STYLE_BASE_PATH, args.subdir)
    content_path = os.path.join(CONTENT_BASE_PATH, args.subdir)
    target_path = os.path.join(TARGET_BASE_PATH, args.subdir)

    print(f"✅ 自动生成路径：")
    print(f"风格图路径：{style_path}")
    print(f"内容图路径：{content_path}")
    print(f"生成图路径：{target_path}\n")

    # 执行计算
    artfid, fid, lpips, lpips_gray = compute_art_fid(
        target_path, style_path, content_path,
        args.batch_size, args.device, args.mode,
        args.content_metric, args.num_workers
    )
